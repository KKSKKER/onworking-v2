...
- **`external`（默认，推荐）** —— AI 能用元数据、schema/配置、管线管理类命令与 CSV 导出（`schema.tables`、`setup.detectSource`、`pipeline.run`、`bigtable.addFiles`、`query.exportCsv` / `bigtable.exportCsv` 等），**读不到也改不到真实业务行数据本身**：`query.run`、`bigtable.previewRows`、`setup.preview`、`setup.exportCsv`（源文件原样导出）等真实数据命令对 AI 一律返回 `AI_MODE_RESTRICTED`；CSV 导出仅跑 SELECT 落盘、不暴露写库，作为交付物放行；破坏性操作（删大表 / 删源文件 / 删映射规则）也仅限人类界面。
...
也就是说：外部 AI 不能直接查数或写库（`query.run` / `bigtable.previewRows` / `setup.preview` 返回 `AI_MODE_RESTRICTED`），破坏性操作仅限人类界面；但 **CSV 导出已对 external 放行**（`query.exportCsv` / `bigtable.exportCsv`，仅 SELECT 落盘），把仓库根目录交给外部 AI 即意味着它可经导出把数据带走——这是有意取舍，部署前请确认可接受。
