...
- **重复表头按列编号映射**：同表头出现 N 次 → 规范名 `姓名_1`..`姓名_N`（与预览/模板/自动映射一致）。**`setup.detectSource` / `getFileHeaders` 返回的 `headers` 就是规范名**——重复表头直接显示 `姓名_1/姓名_2/姓名_3`，照抄进 `mapping.save` 的 `sourceHeader` 即可精确指定列（如 `姓名_2`）。写裸名「姓名」时 `mapping.save` 抛 `MAPPING_DUPLICATE_HEADER`、clean 抛 `CLEAN_DUPLICATE_HEADER`（整个 run 失败 `ok:false`）；报错文案会列出可用编号名，照抄重跑即可。
