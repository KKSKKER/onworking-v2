...
- **跑完管线必读 `warnings` 并汇报**：`pipeline.run` 的返回里有 `warnings` 数组，可能含「跳过无法读取的文件（密码保护/损坏）」「真实行数/列数超上限已截断」「重复表头只取其一」等。**必须把每条告警转告操作者**——被跳过的文件=数据没进、被截断=数据被切，这两类尤其要明确报告，不能只说「跑完了」。
