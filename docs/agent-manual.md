...
- **重复表头按列编号**：同表头出现 N 次 → 规范名 `姓名_1`..`姓名_N`；`getFileHeaders` 返回的 `headers` 就是规范名，照抄进 `mapping.save` 的 `sourceHeader` 即可（如 `姓名_2`）；写裸名「姓名」时 `mapping.save` 报错、clean 整个 run 失败（`ok:false`）并提示编号，照抄重跑即可。
