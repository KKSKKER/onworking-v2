...
- **偏离值证据**：以全表非空行分数的中位数为基线，`deviation = 该行分数 − 基线`。数据行格式一致，偏离小；表头行与周围数据格式差异大，偏离大。偏离越大越可能是表头。
...
**调参**（可选）：数据行里文本注释列很多会抬高基线、让 2 列表头偏离不足 3 被漏掉——把 `deviationFloor` 调低（如 1）放宽；候选太多就把 `limit` 收紧。退化情况（几乎全是字符串、没有稳定的数据行基线）按设计返回空数组，此时回退用 `setup.detectSource`。
