...
// [禁用 2026-08-29] setup.detectHeaders 多表头候选检测接口下线;核心算法 detectHeaderCandidates 仍保留(header-detect.ts + 测试)。
...
//   候选含未确认行、误判风险高,已从接口面下线。恢复时取消注释并接回 handlers.ts / contracts.ts / server.ts。
