...
// 行级血缘:为每行追加来源列,可反查「这个数从哪份文件哪个 sheet 哪一行来」。
...
export const LINEAGE_COLUMNS = ['__source_file', '__source_sheet', '__source_row', '__extracted_at'] as const;
...
export function attachLineage(
  ...
  rows.forEach((row, i) => {
    ...
    row.__source_sheet = source.sourceSheet;
