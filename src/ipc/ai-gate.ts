...
//   - CSV 导出(交付物):query/bigtable 的 exportCsv 仅 SELECT 落盘、不暴露写库,external 放行;setup.exportCsv(源文件原样导出)仍封
...
export const METADATA_ALLOWED = new Set([
  ...
  // CSV 导出(交付物):query.exportCsv / bigtable.exportCsv 仅 SELECT 落盘,不暴露写库能力。
  'query.exportCsv', 'bigtable.exportCsv',
]);
