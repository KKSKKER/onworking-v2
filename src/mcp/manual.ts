...
- 重复表头按列编号:同表头出现 N 次 → 规范名 姓名_1..姓名_N;getFileHeaders 返回的 headers 就是规范名(重复列直接显示 姓名_1/姓名_2/姓名_3),照抄进 mapping.save 的 sourceHeader 即可精确指定列(如 姓名_2);写裸名「姓名」时 mapping.save 报错、clean 整个 run 失败(ok:false)并提示编号,照抄重跑即可。
- 跑完管线必读返回的 warnings 并向操作者汇报:跳过文件(密码保护/损坏=数据没进)、行/列超上限截断(数据被切)、重复表头未按编号映射(run 已失败)。不能只说「跑完了」。
