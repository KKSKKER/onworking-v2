...
//   选中源文件 → setup.preview(展示源文件内容,表头行可调)
...
export function PreviewView() {
  ...
  useEffect(() => {
    ...
    if (selectedFolder) {
      ...
      void loadSource(0, selectedFile);
    }
  ...
  async function loadSource(p: number, filePath: string) {
    setBusy(true);
    const res = await sendCli({ cmd: 'setup.preview', filePath, headerRow, offset: p * PAGE_SIZE, limit: PAGE_SIZE });
    setBusy(false);
    if (!res.ok) { setErr(res.error.message); return; }
    const d = res.data as { headers: string[]; rows: unknown[][]; total: number };
    const rows = d.rows.map((r) => {
      const rec: Record<string, unknown> = {};
      d.headers.forEach((h, j) => { rec[h] = r[j]; });
      return rec;
    });
    setPreview({ title: `文件: ${fileNameOf(filePath)}`, columns: d.headers, rows, total: d.total });
  }

  function handlePageChange(p: number) {
    setPage(p);
    if (selectedFolder) void loadBigTable(p, selectedFolder);
    else if (selectedFile) void loadSource(p, selectedFile);
  }

  function handleHeaderRow(n: number) {
    setHeaderRow(n);
    if (selectedFile) void loadSource(0, selectedFile);
  }
